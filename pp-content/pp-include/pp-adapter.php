...
09a03d6d70021d1c0dd64cefd6e400b18d0e43d00d821b8f52e2e9370908779e',
    ...
    $payrello_favicon= pp_site_url('fulldomain').'/assets/images/favicon-light.png';
    $payrello_logo_light = pp_site_url('fulldomain').'/assets/images/logo-light.png';

    $directory = (pp_site_url('fulldomain') == 'http://localhost') ? 'payrello-panel/' : '';
            ...
            if($action == "system-settings-update-check"){
                if($global_user_login == true){
                    if (!empty($pp_demo_mode)) {
                        ...
                        $manifest = json_decode(file_get_contents('https://updates.payrello.com/manifest.json'), true);

                        $current_code = $payrello_current_version['version_code'];
                        $current_name = $payrello_current_version['version_name'];
                        $version_hash = $payrello_current_version['version_hash'];
            ...
            if($action == "system-settings-update-download"){
                if($global_user_login == true){
                    if (!empty($pp_demo_mode)) {
                        ...
                        if (version_compare($lasted_update_version, $payrello_current_version['version_code'], '>')) {
                            $update_available = true;
                        }

                        if($update_available == true){
                            $url = "https://updates.payrello.com/download.php?version=$lasted_update_version";
                ...
                if($global_user_login == true){
                    if (!empty($pp_demo_mode)) {
                        ...
                        if($update_available == true){
                            ...
                            zipFolder($root, "$backupDir/".$payrello_current_version['version_code'].".zip");

                            backupDatabasePDO("$backupDir/db_".$payrello_current_version['version_code'].".sql");
